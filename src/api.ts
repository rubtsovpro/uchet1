...
api.post('/crm/deals/sync', async (c) => {
  ...
  try {
    ...
    auditFromContext(c, {
      ...
      entity: 'crm_deal',
      summary: `Синк сделок Amo: ${result.deals}, воронок ${result.pipelines}`,
      after: result,
    });
...
api.post('/sync/odata', async (c) => {
  ...
  try {
    ...
    auditFromContext(c, {
      action: 'sync.odata',
      entity: 'sync',
      summary: `OData: складов ${result.warehouses}, категорий ${result.categories}, товаров ${result.products}`,
      after: result,
    });
    return c.json({ ok: true, ...result, hs, hsError, counts: catalogCounts(), hsMeta: hsSyncMeta() });
  } catch (e) {
...
api.post('/sync/hs', async (c) => {
  ...
  try {
    const result = await syncApplicabilityAndProperties();
    auditFromContext(c, {
      action: 'sync.hs',
      entity: 'sync',
...
api.post('/sync/prices', async (c) => {
  ...
  try {
    const result = await syncPricesOnly();
    auditFromContext(c, {
      action: 'sync.prices',
      entity: 'price',
...
api.post('/sync/rests', async (c) => {
  ...
  try {
    const result = await syncRestsOnly();
    auditFromContext(c, {
      action: 'sync.rests',
      entity: 'stock',
...
/* ——— catalogs ——— */
api.get('/categories', (c) => c.json(all('SELECT * FROM categories ORDER BY name')));
...
api.get('/products', (c) => {
  ...
  const total =
    get<{ c: number }>(`SELECT COUNT(*) AS c FROM products p ${where}`, params)?.c ?? 0;
  const items = all(
...
api.get('/products/:id', (c) => {
  ...
  const applicability = all(
    `SELECT mark, model, only_model, generation, years
  ...
  return c.json({ ...product, applicability, properties, prices, media, rests, related });
});
