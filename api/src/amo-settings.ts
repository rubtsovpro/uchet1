...
export function saveAmoIntegrationSettings(
  ...
  writeMeta(next);
  if (patch.pipeline_company !== undefined) {
    applyPipelineCompanyToDeals(next.pipeline_company);
  }
  return next;
}

/** Контур (companies.id) для воронки Amo. */
export function mappedOrgCompanyId(pipelineId: string): string {
  const pipe = String(pipelineId || '').trim();
  if (!pipe) return '';
  return String(getAmoIntegrationSettings().pipeline_company[pipe] || '').trim();
}
...
export function amoBridgePublic() {
  ...
  return {
    bridge: {
      ...
      ingest_key: Boolean(String(process.env.WMS_INGEST_KEY || '').trim()),
      note: 'Ключи Amo хранятся в amo1c. Здесь — маппинг и синк через CLI-мост.',
    ...
    pipelines: pipes.map((p) => ({
      ...
      org_company_id: settings.pipeline_company[String(p.id)] || '',
    ...
    amo_users: (() => {
      ...
      const staffRows = all<{ id: string; name: string; amo_id: string }>(
        `SELECT id, name, IFNULL(amo_id,'') AS amo_id FROM staff WHERE IFNULL(amo_id,'') != ''`
      );
      ...
      return [...byId.values()].sort((a, b) => {
        ...
        return (a.staff_name || a.amo_id).localeCompare(b.staff_name || b.amo_id, 'ru');
      });
    })(),
    /** amo user id из сделок, которых нет ни у кого в staff */
    unmapped_amo_users: (() => {
      const mapped = new Set(
        all<{ amo_id: string }>('SELECT amo_id FROM staff WHERE IFNULL(amo_id,\'\') != \'\'').map(
          (r) => String(r.amo_id)
        )
      );
