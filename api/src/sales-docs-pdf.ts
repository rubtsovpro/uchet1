...
import {
  workorderWarrantyBreakLines,
  workorderWarrantyObligationLines,
...
async function buildWorkorderPdf(
  ...
  return new Promise((resolve, reject) => {
    ...
    const warrantyObl = workorderWarrantyObligationLines(org.inn);
    for (let i = 0; i < warrantyObl.length; i++) {
      doc.text(`${i + 1}. ${warrantyObl[i]}`);
    }
    doc.moveDown(0.25);
    ...
    const warrantyBreak = workorderWarrantyBreakLines(org.inn);
    for (let i = 0; i < warrantyBreak.length; i++) {
      doc.text(`${i + 1}. ${warrantyBreak[i]}`);
    }
    doc.moveDown(0.35);
