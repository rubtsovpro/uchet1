...
import { mountPickRoutes, warmPickTodayCaches } from './modules/pick/routes.js';
export { warmPickTodayCaches };
import {
  createProductionJob,
...
api.get('/health', async (c) => {
  ...
  let bullmq: { ok: boolean; mode: string; waiting?: number } = { ok: true, mode: 'off' };
  try {
    const { bullmqHealth } = await import('./modules/queue/bull.js');
    bullmq = await bullmqHealth();
  } catch {
    /* optional */
  }
  return c.json({
...
api.get('/warehouses/stock-totals', async (c) => {
  const { stockTotalsCacheGet, stockTotalsCacheSet } = await import('./modules/catalog/index.js');
  const cached = await stockTotalsCacheGet<Record<string, unknown>>();
  ...
  await stockTotalsCacheSet(body);
  return c.json(body);
});
...
api.get('/products', async (c) => {
  ...
  const companyId = companyParsed.id;
  /** Autocomplete / suggest: короткий q, 1-я страница, без ids — Redis 8с. */
  const suggestCacheKey =
    ...
    !codeFilter
      ...
      : '';
  if (suggestCacheKey) {
    const { productsSuggestCacheGet } = await import('./modules/catalog/index.js');
    const hit = await productsSuggestCacheGet<Record<string, unknown>>(suggestCacheKey);
  ...
  if (suggestCacheKey) {
    const { productsSuggestCacheSet } = await import('./modules/catalog/index.js');
    await productsSuggestCacheSet(suggestCacheKey, productsBody);
  }
  return c.json(productsBody);
});
