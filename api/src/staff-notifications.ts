...
export async function ensureStaffNotificationsSchema() {
  await run(`
...
export async function createStaffNotification(input: {
  ...
  return await get(`SELECT * FROM staff_notifications WHERE id = ?`, [id]);
}

/** Уведомить ответственного по сделке (amo_id → staff). Fallback — админы. */
export async function notifyDealResponsible(input: {
  deal_id: string;
  kind: string;
  title: string;
  body?: string;
  ...
  const deal = await get<{ responsible_user_id?: string; name?: string }>(
...
export async function listStaffNotifications(staffId: string, opts?: { limit?: number; unread_only?: boolean }) {
  await ensureStaffNotificationsSchema();
...
export async function markStaffNotificationsRead(staffId: string, ids?: string[]) {
  await ensureStaffNotificationsSchema();
